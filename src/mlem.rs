...
mod tests {
    ...
    /// Create a vector of coincidences generated by decays in `rois`
    ...
    /// Rough model of scattered coincidences generated by decays in `rois`.
    ///
    /// The scatters are generated by adjusting trues emanating from the given
    /// `rois`: one end of the true LOR is picked at random, and moved around
    /// the detector ring by a random angle within some limits.
    fn scatter_from_rois(rois: &[&ROI]) -> Vec<LOR> {
        let max_deviation: f32 = 0.1; // turns
        use geometry::Point;
        let mut lors = vec![];
        use rand::prelude::*;
                 ...
                 correction: Option<BuildScattergram>, name: &str)
    {
        ...
        let noise = scatter_from_rois(&[&roi_1, &roi_2, &roi_3, &roi_b]);
